...
Tu analyses des données FBref de la saison 2024-2025.
...
def render_transfer_tab(tabs_ref, filtered, df, ollama_ok, ollama_host, ollama_model, query_ollama_fn):
    ...
    with tabs_ref:
        ...
        data_source = st.radio(
            "Choisir la source",
            ["🔍 Scraper FBref automatiquement (nom du joueur)", "📤 Uploader un CSV FBref", "🎲 Données simulées (démo)"],
            horizontal=True
        )
        ...
        # ── FBref via Playwright (fetch_player.py) ──────────────────────────
        if data_source == "🔍 Scraper FBref automatiquement (nom du joueur)":
            st.markdown("""
            <div style="background:#111827;border:1px solid #2563eb;border-radius:8px;padding:14px;font-size:0.85rem;color:#94a3b8;margin-bottom:12px;">
            <b style="color:#38bdf8">⚡ Étapes pour récupérer les données réelles :</b><br><br>
            <b>1.</b> Installez les dépendances (une seule fois) :<br>
            <code style="background:#0a0d14;padding:4px 8px;border-radius:4px;color:#10b981;">
            pip install playwright beautifulsoup4 pandas<br>
            playwright install chromium
            </code><br><br>
            <b>2.</b> Lancez le script de récupération dans un terminal :<br>
            <code style="background:#0a0d14;padding:4px 8px;border-radius:4px;color:#10b981;">
            python fetch_player.py "Omar Marmoush"
            </code><br><br>
            <b>3.</b> Un fichier CSV est généré → uploadez-le dans l'onglet <b>"📤 Uploader un CSV FBref"</b>
            </div>
            """, unsafe_allow_html=True)

            col_s1, col_s2 = st.columns([3, 1])
            with col_s1:
                player_input = st.text_input("Nom du joueur", value="Omar Marmoush",
                                             placeholder="Ex: Erling Haaland, Kylian Mbappe...")
            with col_s2:
                season = st.selectbox("Saison", ["2024-2025", "2023-2024", "2022-2023"], index=0)
            season_year = int(season.split("-")[0])

            def run_playwright_scraper(name, year):
                """Run fetch_player.py as subprocess and load the resulting CSV."""
                import subprocess, glob, os
                script = Path(__file__).parent / "fetch_player.py"
                if not script.exists():
                    return None, "❌ fetch_player.py introuvable. Placez-le dans le même dossier que app.py."
                try:
                    result = subprocess.run(
                        [sys.executable, str(script), name, "--season", str(year)],
                        capture_output=True, text=True, timeout=90
                    )
                    output = result.stdout + result.stderr
                    if result.returncode != 0:
                        return None, f"❌ Erreur script :\n```\n{output[-800:]}\n```"
                    # Find generated CSV
                    safe = re.sub(r"[^\w\s-]", "", name).strip().replace(" ", "_").lower()
                    csv_path = Path(".") / f"{safe}_{year}.csv"
                    if not csv_path.exists():
                        # search for any matching CSV
                        matches = list(Path(".").glob(f"*{safe}*.csv"))
                        if matches:
                            csv_path = matches[0]
                        else:
                            return None, f"❌ CSV non trouvé après exécution.\nSortie :\n{output[-400:]}"
                    df_result = pd.read_csv(csv_path)
                    return df_result, output
                except subprocess.TimeoutExpired:
                    return None, "❌ Timeout (90s). FBref est peut-être lent. Réessayez."
                except Exception as e:
                    return None, f"❌ Erreur : {e}"

            if st.button("🚀 Lancer la récupération automatique", type="primary", use_container_width=True):
                with st.spinner(f"🌐 Playwright ouvre FBref pour **{player_input}**... (30-60 secondes)"):
                    import sys, re
                    from pathlib import Path
                    result_df, log = run_playwright_scraper(player_input, season_year)
                if result_df is not None:
                    st.session_state['fbref_df']   = result_df
                    st.session_state['fbref_name'] = player_input
                    st.success(f"✅ {len(result_df)} matchs récupérés pour **{player_input}**")
                    with st.expander("📋 Log de récupération"):
                        st.code(log)
                else:
                    st.error(log)
                    st.info("💡 Alternative : utilisez l'onglet **'📤 Uploader un CSV FBref'** avec un fichier téléchargé manuellement.")

            if 'fbref_df' in st.session_state:
                match_df    = st.session_state['fbref_df']
                player_name = st.session_state.get('fbref_name', player_input)
                with st.expander("🔎 Aperçu des données FBref"):
                    st.dataframe(match_df.head(10), use_container_width=True)

        # ── CSV Upload ───────────────────────────────────────────────────────
        elif data_source == "📤 Uploader un CSV FBref":
            uploaded = st.file_uploader("CSV match log FBref", type=["csv"])
            ...
            if uploaded:
                try:
                    ...
                    raw.columns = raw.columns.str.strip().str.lower().str.replace(r'[\s/+]', '_', regex=True).str.replace(r'[^a-z0-9_]', '', regex=True)
                    col_map = {'gls': 'goals', 'ast': 'assists', 'sh': 'shots',
                               'kp': 'key_passes', 'min': 'minutes', 'xg': 'xg', 'date': 'match_date'}
                    ...
                    if 'match_date' in raw.columns:
                        ...
                        raw = raw.dropna(subset=['match_date']).sort_values('match_date')
                    for col in ['goals', 'assists', 'xg', 'shots', 'key_passes', 'minutes']:
                        if col in raw.columns:
                            raw[col] = pd.to_numeric(raw[col], errors='coerce')
                    raw['match_num'] = range(1, len(raw) + 1)
                    raw['played'] = raw.get('minutes', pd.Series([90]*len(raw))).fillna(0) > 0
                    match_df = raw
                    st.success(f"✅ {len(match_df)} matchs chargés")
        ...
        # ── Simulated data ───────────────────────────────────────────────────
            ...
            with col_sim1:
                player_name = st.text_input("Nom du joueur à simuler", value="Omar Marmoush")
        ...
        st.divider()
        st.markdown(f"## ⚽ Analyse de {player_name} — Saison 2024/25")

        avail = [c for c in ['goals','xg','assists','shots','key_passes'] if c in match_df.columns]
        ...
        available_metrics = [c for c in ['goals','xg','assists','shots','key_passes'] if c in played_df.columns]
        ...
        with inner_tabs[0]:
            ...
            <div class="stat-block">
              ...
              • <b>Saison :</b> {match_df.get('season', pd.Series(['2024-2025'])).iloc[0] if 'season' in match_df.columns else '2024-2025'}<br>
        ...
        with inner_tabs[5]:
            ...
            **AR(p)** : la performance du match actuel dépend des **p matchs précédents**.
            ...
            try:
                ...
                model_ar = AutoReg(s_clean, lags=p_ar, old_names=False).fit()
                fitted = model_ar.fittedvalues
                forecast = model_ar.predict(start=len(s_clean), end=len(s_clean)+n_fore-1)
                residuals = model_ar.resid

                eq = f"y(t) = {model_ar.params[0]:.3f} + " + " + ".join([f"{model_ar.params[i+1]:.3f}·y(t-{i+1})" for i in range(min(p_ar,3))])
                if p_ar > 3: eq += " + ..."
                plot_forecast(s_clean, fitted, forecast.values, f"AR({p_ar}) — {metric_choice}", eq, residuals)
                ...
                fore_df = pd.DataFrame({'Match': [f"M+{i+1}" for i in range(n_fore)], f'{metric_choice} prévu': forecast.values.round(4)})
                st.dataframe(fore_df, use_container_width=True, hide_index=True)
            except Exception as e:
                st.error(f"Erreur AR : {e}")
        ...
        with inner_tabs[6]:
            ...
            **MA(q)** : la performance dépend des **q erreurs passées** (chocs aléatoires).
            ...
            try:
                ...
                model_ma = ARIMA(s_clean, order=(0, 0, q_ma)).fit()
                fitted_ma = model_ma.fittedvalues
                forecast_ma = model_ma.forecast(steps=n_fore_ma)
                residuals_ma = model_ma.resid

                eq_ma = f"y(t) = {model_ma.params[0]:.3f} + ε(t)" + "".join([f" + {model_ma.params[i+1]:.3f}·ε(t-{i+1})" for i in range(min(q_ma,3))])
                plot_forecast(s_clean, fitted_ma, forecast_ma.values, f"MA({q_ma}) — {metric_choice}", eq_ma, residuals_ma)
                ...
                with c3:
                    rmse_ma = np.sqrt(np.mean(residuals_ma**2))
                ...
                fore_df_ma = pd.DataFrame({'Match': [f"M+{i+1}" for i in range(n_fore_ma)], f'{metric_choice} prévu': forecast_ma.values.round(4)})
                st.dataframe(fore_df_ma, use_container_width=True, hide_index=True)
            except Exception as e:
                st.error(f"Erreur MA : {e}")
...
"""
            ...
            prompt_ia = f"""
Tu es directeur sportif expert. Voici l'analyse complète de la saison 2024-25 de {player_name}:
...
def main():
    ...
    with col_title:
        ...
        st.markdown('<p class="app-subtitle">FBref 2024-2025 · Stationnarité · Saisonnalité · Ollama AI</p>', unsafe_allow_html=True)
